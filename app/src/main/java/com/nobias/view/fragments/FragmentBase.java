...
import com.nobias.view.activities.ActivityMain;
...
public class FragmentBase extends Fragment {
    ...
    private boolean mIsLocationProgressToShow = false;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mMainActivity = (ActivityMain) getActivity();
        ((MyApplication) mActivity.getApplicationContext()).getAppComponent().inject(this);
    }
    ...
    private void processResultGalleryDocument(int requestCode, Intent data) {
        if (data != null) {
            if (requestCode == REQUEST_PHOTOGALLERY_RESULT) {
                new AsyncImage().execute(data.getData());
            } else {
    ...
    protected void manipulateCameraClick() {
        ...
        } else if (ActivityCompat.checkSelfPermission(mActivity,
                WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            mIsFromPhotoGallery = false;
            mIsFromCamera = true;
            mIsFromDocument = false;
    ...
    private void manipulateCameraCapture() {
        ...
        if (mUriForPhotoResult != null) {
            ...
            String fileSizeError = calculateFileSize(mActivity, file, fileSize, fileSizeMin,
                    fileSizeMax, false);
